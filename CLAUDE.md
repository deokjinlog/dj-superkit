...
# dj-superkit 내부 skill 주의사항

> 위 섹션은 upstream Superpowers 기여 룰. 아래는 dj-superkit 포크 내부 skill 설계 관련 메모.

## ⚠️ 본 CLAUDE.md 의 적용 범위 (꼭 읽고 시작)

이 파일은 **dj-superkit 플러그인 자체를 개발하는 contributor 용 메모**입니다. 플러그인 사용자가 자기 프로젝트에서 dj-superkit 를 쓸 때는 이 파일이 자동 로드되지 **않습니다**. 플러그인 cache 에 같이 들어가긴 하지만 사용자 환경의 CLAUDE.md 만 메인 시야에 들어와요. 그래서 여기에 룰을 박는다고 사용자 환경에 영향이 가는 게 아닙니다.
...
## writing-plans `**Model**:` 필드 ↔ dj-superkit-sub-driven 결합

`writing-plans` 의 task block 신규 `**Model**:` 필드 (v1.1.14+) 는 `dj-superkit-sub-driven` 의 implementer dispatch model 결정에 직접 사용된다 (`skills/dj-superkit-sub-driven/SKILL.md` Plan Analysis & Wave Build 단계). 즉:

- writing-plans 의 평가 룰 (haiku/sonnet/opus 분기) 변경 시 `dj-superkit-sub-driven` 의 dispatch 단계도 동시 수정
...
v1.1.14+ 에서 `scripts/preflight.py` 가 generating-html / code-pretty / executing-plans / dj-superkit-sub-driven 4 skill 의 Pre-flight 검사를 deterministic 코드로 통합. 즉:
...
1. `skills/dj-superkit-sub-driven/implementer-prompt.md` — STRICT BYTE-COPY 룰 + haiku 고정 + Status enum BLOCKED
2. `skills/dj-superkit-sub-driven/reorder-prompt.md` — Status NEEDS_USER 형식 + sonnet 고정 + silent overwrite 차단
3. `scripts/plan_byte_check.py` — `**원본**` 블록 byte-equal 검증 helper (writing-plans + auto-writing-plans 의 Self-Review)
4. `skills/dj-superkit-sub-driven/SKILL.md` — Per-wave Sequence W-2 의 Stage 1/2/3 분기
...
`skills/dj-superkit-sub-driven/tests/H11-user-edit-reorder/README.md` — 사용자 mid-flight 수정 시뮬레이션 + reorder dispatch 발화 검증.
...
`skills/dj-superkit-sub-driven/tests/H12-same-file-merge/README.md` — 같은 파일 4 mechanical 변경 plan → 1 task multi-step 묶음 검증 (positive + negative).
...
`.gitignore` 변경 X (v2.2.0 의 `docs/features/**/*.html` 그대로 + `.dj-superkit/html-regen.log` 는 기존 `.dj-superkit/` glob 흡수).
...
- silent log (`.dj-superkit/html-regen.log`) — gitignored, debug 용
...
v2.3.5+ 에서 `skills/executing-plans/SKILL.md` + `skills/dj-superkit-sub-driven/SKILL.md` + `CLAUDE.md` 3 파일 atomic patch. 한쪽만 변경 시 inline vs subagent 모드 동작 불일치 + 글로벌 vs skill body 룰 불일치.
...
| dj-superkit-sub-driven 만 변경 | 반대 |
...
grep -c "Critical / Non-critical 판정 룰 (v2.3.5+)" \
  skills/executing-plans/SKILL.md skills/dj-superkit-sub-driven/SKILL.md
# expected: 각 1
...
- **B-3 silent log monitor**: `.dj-superkit/html-regen.log` 에 호출 / 성공 / 실패 entry 를 자동 append. `/sync-html --check` 으로 사용자 조회 가능.
...
- **subagent 1** — `dj-superkit-sub-driven` (Wave 별 진행)
...
- 향후 upstream 본문 변경 시 mirror 답습은 그대로. Checklist 섹션만 dj-superkit 고유 추가로 유지
...
for f in skills/auto-brainstorming/SKILL.md skills/auto-tech-design/SKILL.md \
         ...
         skills/dj-superkit-sub-driven/SKILL.md skills/og-writing-plans/SKILL.md \
...
| 한국어 친화 톤 미적용 | dj-superkit 톤 불일치 (v2.4+ A-1~A-5 위배) |
...
- 사용자 환경 출력 (`~/.claude/skills/<slug>/SKILL.md`) — dj-superkit 저장소 외, 사용자가 빌더 실행 시점에 Write
- expand-plan.md (js-ralph) 패턴 답습 — dj-superkit 저장소 안 다른 변경 X
...
- 사용자 환경 출력 — `~/.claude/skills/<slug>/` rename (또는 rm -rf). dj-superkit 저장소 외
...
`/new-skill` 이 생성 시 skill 디렉토리에 `.dj-superkit-skill.json` 마커 파일을 함께 작성한다:

```json
{"generated_by": "dj-superkit:new-skill", "scope": "<project|global>", "created": "<ISO ts>"}
```

판별 규칙: **어떤 skill 이 "dj-superkit 가 만든 것" ⇔ 그 디렉토리에 `.dj-superkit-skill.json` 존재.** `/list-skills` 필터 기준 + `/remove-skill` 차단 기준. 이 규약을 바꾸면 3 command 동시 수정 (한쪽만 바꾸면 생성 마커와 조회/삭제 판별이 desync).
...
| remove-skill § 4-0 차단 게이트 약화 | 비-dj-superkit skill 삭제 가능 → v2.7 핵심 안전성 손상 |
...
```bash
# 3 command 마커 규약 공유
grep -lF ".dj-superkit-skill.json" commands/new-skill.md commands/remove-skill.md commands/list-skills.md
# expected: 3 lines
...
- 사용자 환경 출력 — `<project-root>/.claude/skills/` 또는 `~/.claude/skills/` (dj-superkit 저장소 외)
- `using-superpowers` 본문 변경 X
- 범위 밖: 비-dj-superkit 강제 삭제 우회 / 옛 마커 없는 skill 마이그레이션 / 다른 프로젝트 조회·삭제
