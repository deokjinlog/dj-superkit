...
# intent-locked-workflow 내부 skill 주의사항

> 위 섹션은 upstream Superpowers 기여 룰. 아래는 intent-locked-workflow 포크 내부 skill 설계 관련 메모.

## ⚠️ 본 CLAUDE.md 의 적용 범위 (꼭 읽고 시작)

이 파일은 **intent-locked-workflow 플러그인 자체를 개발하는 contributor 용 메모**입니다. 플러그인 사용자가 자기 프로젝트에서 intent-locked-workflow 를 쓸 때는 이 파일이 자동 로드되지 **않습니다**. 플러그인 cache 에 같이 들어가긴 하지만 사용자 환경의 CLAUDE.md 만 메인 시야에 들어와요. 그래서 여기에 룰을 박는다고 사용자 환경에 영향이 가는 게 아닙니다.
...
v1.1.14+ 에서 `scripts/preflight.py` 가 generating-html / code-pretty / executing-plans / subagent-driven 4 skill 의 Pre-flight 검사를 deterministic 코드로 통합. 즉:
...
intent-locked-workflow 자체 skill 의 Checklist 본문에 박힌 task 명칭은 **사용자 시야 (TaskCreate UI) 에 직접 노출**됨. 다음 룰 적용:
...
- **Gate #14 (실행 모드 선택) override 명시** — v1.1.12+ "자동승인 절대 X" 룰을 auto-executing-plans 가 명시 override. 일반 `/executing-plans` 영향 0 (게이트 그대로). auto-* 명시적 invoke 시에만 작동.
...
1. `skills/subagent-driven/implementer-prompt.md` — STRICT BYTE-COPY 룰 + haiku 고정 + Status enum BLOCKED
2. `skills/subagent-driven/reorder-prompt.md` — Status NEEDS_USER 형식 + sonnet 고정 + silent overwrite 차단
3. `scripts/plan_byte_check.py` — `**원본**` 블록 byte-equal 검증 helper (writing-plans + auto-writing-plans 의 Self-Review)
4. `skills/subagent-driven/SKILL.md` — Per-wave Sequence W-2 의 Stage 1/2/3 분기
...
`skills/subagent-driven/tests/H11-user-edit-reorder/README.md` — 사용자 mid-flight 수정 시뮬레이션 + reorder dispatch 발화 검증.

### 영향 범위

- byte-copy + reorder 는 **subagent 모드에만** 적용 (subagent-driven-development + auto-executing-plans).
- 일반 `/executing-plans` (executing-plans inline) 영향 0 — 사용자 LLM 자율 보정 선호 케이스 보존.
...
`skills/subagent-driven/tests/H12-same-file-merge/README.md` — 같은 파일 4 mechanical 변경 plan → 1 task multi-step 묶음 검증 (positive + negative).

요약: 2 skill + fixture + CLAUDE.md 변경은 묶어서 처리.

## 8 skill AskUserQuestion 강제 룰 결합 (v2.0.3+)

v2.0.3+ 에서 4 intent-locked-workflow 신규 skill + 4 auto-* 변형 skill 의 사용자 질문 시
...
- 8 skill body 변경. og-* / subagent-driven / 보조 skill 영향 0
...
`.gitignore` 변경 X (v2.2.0 의 `docs/features/**/*.html` 그대로 + `.intent-locked/html-regen.log` 는 기존 `.intent-locked/` glob 흡수).
...
- silent log (`.intent-locked/html-regen.log`) — gitignored, debug 용
...
v2.3.5+ 에서 `skills/executing-plans/SKILL.md` + `skills/subagent-driven/SKILL.md` + `CLAUDE.md` 3 파일 atomic patch. 한쪽만 변경 시 inline vs subagent 모드 동작 불일치 + 글로벌 vs skill body 룰 불일치.
...
grep -c "Critical / Non-critical 판정 룰 (v2.3.5+)" \
  skills/executing-plans/SKILL.md skills/subagent-driven/SKILL.md
# expected: 각 1
...
v0.9.0 에서 브레인스토밍~실행 워크플로우의 "얇은 명령 wrapper" 8개를 제거하고 스킬 단일 진입으로 전환. 원조 superpowers 철학(스킬 중심)에 정렬. 근거: 스킬이 이미 슬래시 호출 + 자동 발동 둘 다 하므로 wrapper 명령은 기능상 불필요 (유일한 값은 `/intent-locked-workflow:이름` 접두사 표시뿐).
...
| /tech-design | /tech-design (스킬 이름 동일 — 슬래시 유지) |
| /write-plan | /writing-plans |
| /execute-plan | /executing-plans |
| /auto-brainstorm | /auto-brainstorming |
| /auto-tech-design | /auto-tech-design (동일) |
| /auto-write-plan | /auto-writing-plans |
| /auto-execute-plan | /auto-executing-plans |

### 유지 명령 (일부러 명령 — 자동 발동 차단 / 원본 접근 / 유틸)

- 빌더 3 (list-skills / new-skill / remove-skill) — 자동 발동 시 스킬 생성·삭제 사고 방지 (META-BUILDER 룰)
- og-* 3 (og-brainstorm / og-write-plan / og-execute-plan) — upstream 원본 명시 접근
- 유틸: fast-tasks (짝 스킬 없음) / api-test / sync-html / pretty-md / audit-*

### 참조 일괄 치환 + 미러 보존

- README + CLAUDE.md + skills/*/SKILL.md + 남은 commands/*.md 에서 삭제된 6개 슬래시(`/brainstorm` `/write-plan` `/execute-plan` `/auto-brainstorm` 등)를 스킬 이름으로 일괄 치환. `/tech-design` `/auto-tech-design` 는 스킬 이름이 같아 무변경.
- og-* 스킬 3개 본문 변경 0 (미러 룰 보존 — 짝 슬래시 없어 no-op).
- docs/features(변경이력) · RELEASE-NOTES.md(upstream 과거)는 미변경 (역사 보존). SETUP-WSL.md 스모크 테스트만 `/brainstorming` 으로 갱신.

### 회귀 catch grep

```bash
# CLAUDE.md 는 제외 — 본 메모가 삭제된 명령 이름을 표로 문서화하므로 매치됨 (정상)
grep -rE "/(brainstorm|write-plan|execute-plan|auto-brainstorm|auto-write-plan|auto-execute-plan)\b" \
  README.md SETUP-WSL.md skills/*/SKILL.md commands/*.md
# expected: 0 (스킬 이름 /brainstorming, /writing-plans 등만 존재)
...
intent-locked-workflow 의 사용자 노출 안내문 (메인이 사용자에게 직접 보여주는 모든 문구) 은 다음 룰을 따른다.
...
- **B-3 silent log monitor**: `.intent-locked/html-regen.log` 에 호출 / 성공 / 실패 entry 를 자동 append. `/sync-html --check` 으로 사용자 조회 가능.
...
- **subagent 1** — `subagent-driven` (Wave 별 진행)
...
- 향후 upstream 본문 변경 시 mirror 답습은 그대로. Checklist 섹션만 intent-locked-workflow 고유 추가로 유지
...
for f in skills/auto-brainstorming/SKILL.md skills/auto-tech-design/SKILL.md \
         ...
         skills/subagent-driven/SKILL.md skills/og-writing-plans/SKILL.md \
...
grep -lF "## Checklist" \
  ...
  skills/subagent-driven/SKILL.md \
  skills/og-writing-plans/SKILL.md \
  skills/og-executing-plans/SKILL.md
