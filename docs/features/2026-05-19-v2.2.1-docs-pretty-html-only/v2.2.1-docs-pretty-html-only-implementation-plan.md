...
**Tech Stack:** Markdown skill body, Sonnet subagent (B), Task tool `run_in_background=true`, `.dj-superkit/html-regen.log` silent log.
...
5. 완료 notify 받으면 `.dj-superkit/html-regen.log` silent log
...
| R6 | breaking | `skills/docs-pretty/SKILL.md` Step 3 silent log 위치 명시 (Task 5 Step 4) | `.dj-superkit/html-regen.log` 고정 + 기존 gitignore glob 흡수 (`.gitignore` 변경 X) |
...
## 3. Wave-Parallel 분석 (dj-superkit-sub-driven용)
...
- `[코드-수정]` CH-20260519-002 / 2026-05-19 / 이유: Task 1 신규 `/regen-html` slash command / 무엇이: `commands/regen-html.md` 신규 (fire-and-forget B dispatch 명시 호출, latest slug 추론, 디바운스 3초 cancel) / 영향범위: 사용자 명시 호출 + `change-propagation` 자동 호출 진입점
- `[코드-수정]` CH-20260519-003 / 2026-05-19 / 이유: Task 2 `change-propagation` 자동 호출 룰 추가 / 무엇이: `skills/change-propagation/SKILL.md` Acceptance 5번 + Related Skills 끝 라인 추가 / 영향범위: live doc 진입 후 `.html` stale 방지
- `[코드-수정]` CH-20260519-004 / 2026-05-19 / 이유: Task 4 H18 fixture 신규 (race / 디바운스 / 실패 시나리오) / 무엇이: `skills/docs-pretty/tests/H18-html-fire-and-forget/README.md` 신규 (5 시나리오 + 검증 체크리스트 + Anti-Patterns 회귀 catch) / 영향범위: 다음 외부 피처 dogfood 시 검증 발현
- `[코드-수정]` CH-20260519-005 / 2026-05-19 / 이유: Task 5 `docs-pretty/SKILL.md` Procedure 분기 변경 (병렬 → 단일 fire-and-forget) / 무엇이: frontmatter description + Why fire-and-forget B 섹션 + Step 2 (run_in_background=true + 디바운스) + Step 3 (즉시 return + silent log) + Anti-Patterns 4 라인 / 영향범위: 모든 docs-pretty 발동에 fire-and-forget 적용
- `[코드-수정]` CH-20260519-006 / 2026-05-19 / 이유: Task 3 H17 fixture patch (v2.2.0 → v2.2.1) / 무엇이: 시나리오 단일 fire-and-forget 으로 갱신 + latency < 100ms + 푸터 stale 표시 + 실패 모드 표 갱신 / 영향범위: H17 자체 검증 시나리오 일관성
- `[코드-수정]` CH-20260519-007 / 2026-05-19 / 이유: Task 6 CLAUDE.md 결합 메모 갱신 / 무엇이: `## docs-pretty .html companion 결합 (v2.2.0 → v2.2.1+)` 섹션 (5 파일 atomic patch 룰 + 9 회귀 패턴 + 영향 범위 + 3 신규 catch grep) / 영향범위: 향후 patch 의 결합 catch 라인
- `[코드-수정]` CH-20260519-008 / 2026-05-19 / 이유: Task 7 manifest 6 파일 version bump / 무엇이: 6 manifest 2.2.0 → 2.2.1 (`scripts/bump-version.sh 2.2.1` 일괄, marketplace.json + gemini-extension.json 수동 보완) / 영향범위: marketplace + 다른 harness manifest 동기
