...
<HARD-GATE>
You MUST have an existing <slug>-requirements.md in the current feature folder before invoking this skill. If none exists, instruct the user to run /brainstorming first.
</HARD-GATE>
...
7. **초안 검토 및 승인** — show the full RAW `<slug>-tech-design.md` AND the verify-spec report in one message; ask once "Approve and proceed? — yes / no". On `no` → revise → loop back to step 4 (Self-review → re-verify → re-show the RAW `.md`). Stops once first change-history entry is logged.
8. **변경이력 기록** — append first `[개발방향-수정]` entry via `change-history` skill
9. **다음 단계 진입 확인** — change-history 직후 사용자에게 명시적 yes/no 게이트. On `yes` → invoke `writing-plans` via Skill tool. On `no` → exit with notice telling the user to run /writing-plans later. (v1.1.12+ — restored)
...
> **다음 단계 안내**: 이 문서는 기술 설계서입니다 (아키텍처 / 컴포넌트 / 데이터 / 인터페이스 / 결정 / 위험 / 테스트 전략). `<slug>-requirements.md` (PRD) 를 기반으로 작성되고, 다음 단계 `<slug>-implementation-plan.md` (단계별 계획) 의 입력이 됩니다. 다음 단계로 `writing-plans` skill (또는 `/writing-plans` 슬래시) 을 호출해서 구현 계획을 만드세요. 단계별 구현 task 는 여기 박지 마세요 — 그건 다음 산출물 (plan) 에 들어갑니다.
...
digraph design_flow {
    ...
    "Exit: tell user to run /writing-plans later" [shape=oval];
    ...
    "Single combined approval gate\n(RAW doc + verify report)" -> "Self-review (internal)" [label="no — revise → re-show"];
    ...
    "Ask: proceed to writing-plans? (Gate #12, v1.1.12+ restored)" -> "Exit: tell user to run /writing-plans later" [label="no"];
}
```
...
**1. Verify input**
- Confirm <slug>-requirements.md exists in the same feature folder. If not, HARD-GATE — instruct the user to run `/brainstorming` first.
...
- Re-fires on each user-fix iteration (per-draft loop): revise RAW → generating-html → re-show the RAW `.md`
...
```json
{
  ...
  "context": "RAW doc + 4축 보고서 한 메시지로 노출됨",
  "choices": [
...
```json
{
  ...
  "choices": [
    {"value": "yes", "label": "예 — /writing-plans 자동 invoke"},
    {"value": "no", "label": "아니오 — 종료, 나중에 /writing-plans 수동 실행"}
  ]
}
```
...
- On `no` → emit `ℹ️ 알겠습니다. /writing-plans 은 나중에 직접 실행해주세요.` and stop.
...
3. **Single combined approval gate** — present in ONE message:
   ...
   - User reviews RAW markdown. generating-html already fired before this gate.
...
5. **Proceed-to-writing-plans gate** (v1.1.12+ restored):
   ...
   **Gate #12 — proceed-to-writing-plans** — see Tool form + Prose fallback above (step 9 in the main Process detail).
   ...
   On `yes` → invoke writing-plans via Skill tool. On `no` → emit `ℹ️ 알겠습니다. /writing-plans 은 나중에 직접 실행해주세요.` and stop.
