...
dj-superkit' brainstorming is restricted to **planning-level requirements output**. Technical design and implementation plans are handled by `tech-design` and `writing-plans` skills respectively.
...
After change-history entry is logged, **automatically invoke** the `tech-design` skill (or `dj-superkit:tech-design` depending on harness namespace). NO user gate here.
...
```
ℹ️ '<감지된 키워드 / 신호>' 신호로 og-brainstorming 으로 자동 전환합니다. 되돌리려면 "dj-superkit" 라고 답해주세요.
```
...
| "spec.md is fine, isn't it?" | dj-superkit separates PRD from technical spec. The file is <slug>-requirements.md, not spec.md. |
