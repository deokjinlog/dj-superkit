...
intent-locked-workflow' brainstorming is restricted to **planning-level requirements output**. Technical design and implementation plans are handled by `tech-design` and `writing-plans` skills respectively.
...
0. **Entry Router (v1.1.15+, FR-3)** — 사용자 입력에 명시적 small 신호 감지 시 즉시 og-brainstorming auto-invoke + notice 한 줄. 그 외 → AskUserQuestion 게이트 (og- / intent-locked-workflow 양자택일). 자세한 룰은 "Entry Router" 섹션 참조.
...
digraph brainstorm_flow {
    ...
    "AskUserQuestion 게이트\n(og / intent-locked-workflow)" [shape=diamond];
    ...
    "AskUserQuestion 게이트\n(og / intent-locked-workflow)" -> "Auto-invoke og-brainstorming\n+ notice" [label="og"];
    "AskUserQuestion 게이트\n(og / intent-locked-workflow)" -> "Explore project context" [label="intent-locked-workflow"];
...
After change-history entry is logged, **automatically invoke** the `tech-design` skill (or `intent-locked-workflow:tech-design` depending on harness namespace). NO user gate here.
...
intent-locked-workflow:brainstorming 진입 시 1순위 발화. `/brainstorming` slash command 진입 / 자연어 진입 ("…를 만들어 / 브레인스토밍 시작해") 모두 동일 path.
...
```
ℹ️ '<감지된 키워드 / 신호>' 신호로 og-brainstorming 으로 자동 전환합니다. 되돌리려면 "intent-locked-workflow" 라고 답해주세요.
```

사용자가 "intent-locked-workflow" 라고 응답하면 라우터 무시하고 본 skill 의 Checklist 1번 (Explore) 으로 진입.
...
```json
{
  ...
  "options": [
    ...
    {"label": "intent-locked-workflow:brainstorming", "description": "3-MD 풀 트랙 / PRD + tech-design + plan / 변경이력 + 위험 주석"}
  ]
}
```

사용자 선택 → og 면 og-brainstorming Skill invoke / intent-locked-workflow 면 본 skill Checklist 1번 진입.
...
| "spec.md is fine, isn't it?" | intent-locked-workflow separates PRD from technical spec. The file is <slug>-requirements.md, not spec.md. |
