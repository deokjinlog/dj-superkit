# H18 — generating-html fire-and-forget race / 디바운스 / 실패 시나리오 (메타 dogfood)
...
1. `.md` 작성 → `generating-html` 발동 → B subagent dispatch (`run_in_background=true`)
2. 메인 turn 즉시 return (latency < 100ms)
3. **사용자가 즉시 `.html` 열려고 시도** → 부재 또는 빈 파일
4. 기대 동작: 사용자가 인지 → `/sync-html <slug>` 수동 호출 → 재생성
...
4. **마지막 단계에 `/sync-html <slug>` 자동 호출** (v2.2.1+ Acceptance 5번)
5. `.html` 자동 재생성 (fire-and-forget, 메인 대기 X)

## 시나리오 5 — `.md` 직접 편집 후 사용자 수동 `/sync-html`

1. live doc 상태
2. 사용자가 오타 한두 글자 등 `.md` 직접 편집
3. `generating-html` 발동 X (boundary 보존) + `change-propagation` 호출 안 됨 → `.html` stale
4. 사용자가 `.html` 푸터의 stale 표시 인지 → `/sync-html <slug>` 명시 호출
5. `.html` 재생성
...
```bash
# 메인이 generating-html 발동 전후 turn elapsed 차이
