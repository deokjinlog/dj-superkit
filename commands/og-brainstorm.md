---
description: upstream superpowers 원본 brainstorming(자유 탐색 Socratic 방식)을 호출합니다. intent-locked-workflow 확장(PRD/변경이력/위험주석/generating-html)을 우회하고 원본 그대로의 경험을 원할 때 사용.
---
...
| 항목 | `/brainstorming` (intent-locked-workflow 확장) | `/og-brainstorm` (upstream 원본) |
|---|---|---|
...
`/og-*` 흐름과 `/brainstorming`, `/tech-design`, `/writing-plans`, `/executing-plans` 흐름을 **하나의 피처 안에서 섞지 마세요**. 한 피처는 한 흐름으로 일관되게.
