...
| 다음 단계 | `/design` (designing-direction) | `/og-write-plan` (og-writing-plans) |
...
`/og-*` 흐름과 `/brainstorm`, `/design`, `/write-plan`, `/execute-plan` 흐름을 **하나의 피처 안에서 섞지 마세요**. 한 피처는 한 흐름으로 일관되게.
