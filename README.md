...
js-super 1개만 깔고도 upstream 원본 워크플로우에 접근 가능하도록 **`og-`(original) 접두사 사본** 을 제공한다. js-super 확장이 무겁다고 느낄 때 (예: 내부 스크립트, 탐색 작업) upstream 원본 동작으로 도망갈 수 있다.
...
다른 upstream skill (subagent-driven-development, finishing-a-development-branch, debugging 등)은 js-super가 손대지 않은 채로 vendor되어 있어 원본 이름 그대로 동작 — `og-` 사본 별도로 없다.
